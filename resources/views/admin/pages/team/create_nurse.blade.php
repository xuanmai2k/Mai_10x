...
@section('content')
    <div class="container mt-3" style="margin-bottom: 2%" >
        <h2>Nurse</h2>
        <form action="{{ route('admin.nurse.store') }}" role="form" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-3 mt-3">
                <label>Name:</label>
                <input type="text" class="form-control" id="name" placeholder="Enter name nurse" name="name">
            </div>
            @error('name')
